...
class SLmodel():
	...
	def __init__(self, nx, ns, nh, npcl, xvar=1.0):
		...
		init_A=np.asarray(np.random.randn(ns,nh)/10.0,dtype='float32')
		...
		self.rel_lrates=np.asarray([  0.1,    1.0,    0.01,   10.0,    1.0,     1.0]   ,dtype='float32')
	...
	def sample_proposal_s(self, s, h, xp):
		...
		return s_prop, s_pred, prop_term, prop_mean
	...
	def update_params(self, x1, x2, n_samps, lrate):
		...
		h_probs=self.calc_h_probs(self.s_past)
		
		sp=self.get_prediction(self.s_past, h_probs)
		...
		h1_probs=self.calc_h_probs(s1_samps)
		
		s_pred = self.get_prediction(s1_samps, h1_probs)
